angular.module('ngDraughts', [])

  .controller('DraughtsCtrl', function($scope, $timeout) { 
    var WHITE = "White", BLACK = "Black", BOARD_WIDTH = 8,
        selectedSquare = null;
      ...
      for (var i = 0; i < BOARD_WIDTH; i++) {
        ...
        for (var j = 0; j < BOARD_WIDTH; j++) {
          if ( (i === 0 &&  j % 2 === 0) || (i === 1 && j % 2 === 1) ) {
            ...
            $scope.board[i][j] = new Piece(WHITE, j, i);
          } else {
  ...
  //Any piece that reaches the far edge of the board is crowned and is refered to as a king
    ...
    function setChoices(x, y, depth, matados, oldX, oldY, isKing) {
      ...
      isKing = 
          isKing || 
          ($scope.player === WHITE && y == 0) || 
          ($scope.player === BLACK && y == BOARD_WIDTH - 1);
      // Upper Choices
      if ($scope.player === WHITE || isKing) {
        // Upper Left
        if (x > 0 && y > 0) {
